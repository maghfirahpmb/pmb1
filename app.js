...
function applyRemoteResetIfNeeded(state){
  ...
  if(resetExamKey){
    saveResults(getLocalResults().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no || r.examKey!==resetExamKey));
    saveRemoteResults(getRemoteResultsCache().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no || r.examKey!==resetExamKey));
    removeExamDrafts(no, resetExamKey);
  }else{
...
function removeParticipantExamFromLocalCaches(noUjian, examKey){
  const no=normalizeNoUjian(noUjian);
  saveResults(getLocalResults().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no || r.examKey!==examKey));
  saveRemoteResults(getRemoteResultsCache().filter(r=>normalizeNoUjian(r.noUjian || r.username || r.name)!==no || r.examKey!==examKey));
